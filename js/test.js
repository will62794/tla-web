...
async function testStateGraphEquiv(testId, stateGraph, parsedSpec, specPath, constvals, spec) {
    ...
    console.log("nodeGraph:", interp.initEvalGraph);
    displayEvalGraph(interp.initEvalGraph, "init-eval-graph-pane");
    displayEvalGraph(interp.nextEvalGraph, "next-eval-graph-pane");
...
(async () => {
    ...
    function fetchTestSpec(test, onCompleteFn) {
        ...
        return Promise.all([fetchGraph,fetchSpec]).then(function([specStateGraph, specText]){
            ...
            spec.parse().then(function () {
                ...
                return testStateGraphEquiv(test["spec"], specStateGraph, parsedSpec, specPath, test["constvals"], spec)
    ...
    if(testsToRun instanceof Array){
        ...
        // Hide eval graphs when running multiple tests.
    }
